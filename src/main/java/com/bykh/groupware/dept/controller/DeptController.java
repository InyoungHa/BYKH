...
public class DeptController {
	...
	public String organizationManage(Model model) {
		...
		/*
		 * //지역 목록 조회 List<OrganizationVO> organizationList = deptService.getLocList();
		 * 
		 * //모든 지역 정보를 조회 for(int i = 0 ; i < organizationList.size() ; i++) {
		 * OrganizationVO organizationVO = organizationList.get(i);
		 * 
		 * //지역에 속한 모든 부서 목록 조회 List<OrgDeptVO> deptList =
		 * deptService.getDeptListForOrg(organizationVO.getLoc());
		 * 
		 * //모든 부서에 포함된 모든 직원 조회 for(int j = 0 ; j < deptList.size() ; j++) { int deptno
		 * = deptList.get(j).getDeptno(); List<EmpVO> empList =
		 * deptService.getEmpListForOrg(deptno); deptList.get(j).setEmpList(empList); }
		 * 
		 * 
